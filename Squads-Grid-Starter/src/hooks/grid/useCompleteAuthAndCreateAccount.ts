...
export default function useCompleteAuthAndCreateAccount() {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<GridError | null>(null)

  const completeAuthAndCreateAccount = useCallback(async (email: string, otp: string) => {
    setLoading(true)
    setError(null)
    try {
      const sessionSecrets: SessionSecrets = load("sessionSecrets")!
      return await gridClient.completeAuthAndCreateAccount({
        otpCode: otp,
        user: { email },
        sessionSecrets,
      })
